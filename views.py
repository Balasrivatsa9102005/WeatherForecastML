...
def get_current_weather(city):
    ...
    try:
        ...
        return {
            ...
            'wind_gust_speed': round(data['wind']['speed'], 1),
            'clouds': data['clouds']['all'],
            'visibility': round(data['visibility'] / 1000, 1)  # Convert meters to km
        }
    except requests.exceptions.RequestException as e:
        print(f"[Error] Network/API issue: {e}")
        return None
    except KeyError as ke:
        print(f"[Error] Missing key in response: {ke}")
        return None
...
def read_historical_data(filename):
    try:
        ...
        print(f"[Error] Problem parsing '{filename}'.")
        return None
...
def train_rain_model(X,y):
    ...
    print("Accuracy for Rain Prediction Model:")
...
def weather_view(request):
    if request.method == 'POST':
        ...
        if not current_weather:
            return render(request, 'weather.html', {'error': 'Could not fetch weather data. Try again.'})
        ...
        if historical_data is None:
            return render(request, 'weather.html', {'error': 'Historical data not found or corrupted.'})
        ...
        for i in range(5):
            forecast.append({
                ...
                'hum': round(future_humidity[i], 1)  # key must be 'hum' for template
            })
